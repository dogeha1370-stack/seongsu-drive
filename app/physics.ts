...
export function integrate(p:Body,dt:number,blocked:(x:number,z:number,r:number)=>boolean){p.stun=Math.max(0,p.stun-dt);const oldX=p.x,oldZ=p.z;const nx=p.x+p.vx*dt,nz=p.z+p.vz*dt;if(!blocked(nx,oldZ,2.3))p.x=nx;else{p.vx*=-.3;p.spin*=.6}if(!blocked(p.x,nz,2.3))p.z=nz;else{p.vz*=-.3;p.spin*=.6}p.angle+=p.spin*dt;p.spin*=Math.exp(-2.6*dt);const drag=Math.exp(-.32*dt);p.vx*=drag;p.vz*=drag;if(!Number.isFinite(p.x+p.z+p.angle)){p.x=oldX;p.z=oldZ;p.vx=p.vz=p.spin=0;p.angle=0}}
